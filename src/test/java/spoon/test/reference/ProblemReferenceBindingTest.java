...
class ProblemReferenceBindingTest {
	...
	@Test
	void conditionalAnonymousResourceCanBeUsedAsAMessageArgument() {
		// contract: inferred anonymous resources nested in invocation arguments retain their declared type
		// (SPOON-ISSUE-PLACEHOLDER-01)
		// given
		Launcher launcher = launcherForAnonymousResourceProblemBinding();
		...
		// then
		CtInvocation<?> invocation = invocationNamed(model, "consumeConditional");
	...
	@Test
	void conditionalAnonymousResourceKeepsAResolvedObjectParameter() {
		// contract: recovery does not replace a resolved executable's declared parameter type
		// (SPOON-ISSUE-PLACEHOLDER-01)
		// given
		Launcher launcher = launcherForAnonymousResourceProblemBinding();
		...
		// then
		CtInvocation<?> invocation = invocationNamed(model, "acceptObject");
		assertThat(invocation.getExecutable().getParameters())
				.singleElement()
	...
	@Test
	void conditionalAnonymousResourceKeepsAnInapplicableMethodsClosestMatch() {
		// contract: recovery preserves a known method signature after an applicability failure
		// (SPOON-ISSUE-PLACEHOLDER-01)
		// given
		Launcher launcher = launcherForAnonymousResourceProblemBinding();
		...
		// then
		CtInvocation<?> invocation = invocationNamed(model, "acceptString");
		assertThat(invocation.getExecutable().getParameters())
				.singleElement()
				.extracting(spoon.reflect.reference.CtTypeReference::getQualifiedName)
	...
	@Test
	void conditionalAnonymousResourceKeepsAnInapplicableConstructorsClosestMatch() {
		// contract: recovery preserves a known constructor signature after an applicability failure
		// (SPOON-ISSUE-PLACEHOLDER-01)
		// given
		Launcher launcher = launcherForAnonymousResourceProblemBinding();
		...
		// then
		assertThat(constructorParameter(model, "KnownStringWrapper").getQualifiedName())
				.isEqualTo("java.lang.String");
	}

	@Test
	void differentConditionalAnonymousResourcesKeepTheirCommonFallbackType() {
		// contract: recovery does not choose one branch when a conditional has different resource types
		// (SPOON-ISSUE-PLACEHOLDER-01)
		// given
		Launcher launcher = launcherForAnonymousResourceProblemBinding();
		...
		// then
		assertThat(constructorParameter(model, "MissingMixedWrapper").getQualifiedName())
				.isEqualTo("java.lang.Object");
	}
	...
	void nullConditionalBranchesKeepTheAnonymousResourceType() {
		...
		// then
		assertThat(constructorParameter(model, "MissingNullableWrapper").getSimpleName())
				.isEqualTo("MissingResource");
		assertThat(constructorParameter(model, "MissingNullFirstWrapper").getSimpleName())
				.isEqualTo("MissingResource");
	}
